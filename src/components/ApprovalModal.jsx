...
const ApprovalModal = ({ isOpen, onClose, entry }) => {
    ...
    const handleSubmit = () => {
        const payload = isNegative ? {} : { unit_cost: Number(unitValue) };
        approveTransactionMutation.mutate(JSON.stringify(payload));
    };
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-md border border-slate-200 dark:border-slate-800">
                ...
                <div className="p-6 space-y-4">
                    ...
                    {isNegative ? (
                        <div className="p-3 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 rounded-lg text-xs leading-normal">
                            ℹ️ This is a stock deduction (negative quantity). The unit cost of the deducted quantity will be calculated automatically by the backend using the raw material's latest price.
                        </div>
                    ) : (
                        <>
                            <div>
